...
### grep filter
~~~script
#will not show the file name with [SPAEC]
ls ./ | grep -E "\.h|\.txt" | grep -v " "
~~~
...
function cmd_2_array(){
	...
	for((i=1; i<=$count; i++)); do
		CMDOUT_ARRAY[$i - 1]=$(sed -n "${i}p" data)
	done
...
function voice_proc(){
	...
	file-process $FILE_CONFIG $driver_header
	check_exit $? "cirrus script error"

	#copy new tuning file to the right directory
	copy-voc-tuning $driver_header 
	rm $driver_header

	#modify the header file to include new tuning
	voice-header-insert $(basename $FILE_CONFIG) $driver_header
}
~~~
###getopts
~~~script
echo "OPTIND starts at $OPTIND"
while getopts ":p:v:hm:l:" optname
do
    case "$optname" in
    "p")
...
~~~script
~~~
script for cspl tuning on X30 MTK platform
~~~script
#!/bin/bash
...
function playback_args_proc(){
	if [ $CFG_PLAYBACK = "null" ]; then
		if [ -n "$DIR_SRC" ]; then
			#TODO Check space in the file name
			LS_LIST=$(ls -t "$DIR_SRC" |grep -E "\.txt|\.h" | grep -v "* *")
